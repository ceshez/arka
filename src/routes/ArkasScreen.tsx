...
export function ArkasScreen() {
  const arkaRecords = useArkaStore((state) => state.arkas)
  const arkas = arkaRecords.map((arka) => ({ ...arka, memberCount: arka.members.length }))
  const recentArkas = arkas
  const activeArkas = arkas.filter((arka) => arka.status !== 'completed' && arka.status !== 'cancelled')
  const totalCollected = arkas.reduce((total, arka) => total + calculateArkaProgress(arka).collectedFiat, 0)

  return (
    <MobileScreen>
      <ScreenContainer>
        <HomeArkasHeader title="Arkas" subtitle="Open and recent shared tabs" />
        ...
        >
          ...
          <p className="mt-2 text-xs font-black text-[#f7c842]">Confirmed contributions across your Arkas</p>
            ...
            <Link className="block" key={arka.id} to={arka.status === 'completed' ? `/arka/${arka.id}/completed` : `/arka/${arka.id}/host/summary`}>
              <Card className="space-y-3 p-4">
                ...
                <div className="flex items-center justify-between text-sm font-semibold text-arka-muted">
                  <span>{formatUsd(progress.collectedFiat)} confirmed</span>
