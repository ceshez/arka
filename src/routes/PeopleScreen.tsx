...
import { demoContacts, type DemoContact } from '../data/demoContacts'
...
export function PeopleScreen() {
  ...
  }, [nicknames, query, sortMode])

  const openNickname = (contact: DemoContact) => {
    setSelected(contact)
  ...
  return (
    <MobileScreen>
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}>
        <ScreenContainer>
          <ArkaHeader title="People" subtitle="Everyone you have shared an Arka with" backTo="/" />

          <div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-arka-muted" size={19} /><input value={query} onChange={(event) => setQuery(event.target.value)} className="min-h-13 w-full rounded-xl border border-[#e2d9cc] bg-white pl-11 pr-4 text-sm font-semibold outline-none focus:border-[#b57a00]" placeholder="Search people or nicknames" aria-label="Search people" /></div>

          <div className="flex gap-2 overflow-x-auto pb-1" aria-label="Sort people">
          ...
          <section className="overflow-hidden rounded-xl border border-[#e6ded2] bg-white">
            ...
            {!contacts.length ? <div className="grid min-h-48 place-items-center px-6 text-center"><div><UsersRound className="mx-auto text-arka-muted" /><p className="mt-3 text-sm font-black">No people found</p><p className="mt-1 text-xs font-semibold text-arka-muted">Try another name or nickname.</p></div></div> : null}
          </section>
          <p className="flex items-center justify-center gap-2 text-xs font-semibold text-arka-muted"><ArrowDownUp size={14} /> Nicknames are private and only visible to you.</p>
        </ScreenContainer>
      </motion.div>
