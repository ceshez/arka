...
export function WalletReconnectNotice() {
  ...
  if (wallet || !lastConnectedAddress) return null
  ...
  return (
    ...
    >
      <span className="relative size-11 shrink-0">
        <MemberIdenticon seed={lastConnectedAddress} className="size-11 border-2 border-white shadow-none" />
      ...
      <span className="min-w-0 flex-1">
        <strong className="block text-sm font-black">Wallet disconnected</strong>
        <span className="block text-xs font-semibold leading-4 text-arka-muted">
          {error || 'Connect to keep paying with NIM.'}
        </span>
      </span>
      <button
        type="button"
        ...
        disabled={connecting}
        className="pointer-events-auto inline-flex min-h-10 shrink-0 items-center justify-center gap-1.5 rounded-xl bg-[#7d5700] px-3 text-xs font-black text-white transition active:scale-95 disabled:opacity-65"
      >
